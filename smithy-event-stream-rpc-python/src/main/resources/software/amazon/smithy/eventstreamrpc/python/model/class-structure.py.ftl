...
<#-- repr -->
<#-- Members are sensitive when @sensitive is on the structure, the member, or its target shape.
     Their values are redacted from __repr__ so they don't leak into logs. -->
<#assign sensitiveNames = []>
<#list memberShapes as memberShape>
    <#assign memberTargetShape = context.getShape(memberShape.getTarget())>
    <#if structure.hasTrait("sensitive") || memberShape.hasTrait("sensitive") || memberTargetShape.hasTrait("sensitive")>
