...
public final class NameUtils {
    ...
    public static String capitalizeAcronymSubstring(String str) {
        final StringBuilder stringBuilder = new StringBuilder();
        int acronymStart = -1, acronymEnd = 0;
        for (int i=0; i < str.length(); i++){
            if (i < str.length()-1 &&
                Character.isUpperCase(str.charAt(i)) &&
                Character.isUpperCase(str.charAt(i+1))) {
                if(acronymStart == -1) acronymStart = i;
                acronymEnd = i + 1;
            } else {
                if(acronymStart != -1) {
                    // append the last acronym
                    stringBuilder.append(capitalize(str.substring(acronymStart, acronymEnd+1).toLowerCase()));
                } else {
    ...
    public static String prependMemberPrefix(String str) {
        return new String("m_" + uncapitalize(str));
    }
    ...
    public static String camelToConstantCase(final String label) {
        return camelToSnakeCase(label).toUpperCase();
    }

    public static String camelToSnakeLower(final String label) {
        return camelToSnakeCase(label).toLowerCase();
    }

    public static String camelToSnakeCase(final String label) {
        final StringBuilder stringBuilder = new StringBuilder();
