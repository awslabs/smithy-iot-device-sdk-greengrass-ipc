...
public class StringEnumBuilder implements Function<StringShape, JavaFile> {
    ...
    public JavaFile apply(StringShape stringShape) {
        ...
        enumBuilder.addField(FieldSpec.builder(
                ParameterizedTypeName.get(ClassName.get(Map.class), ClassName.get(String.class), className), "lookup")
                .addModifiers(Modifier.PRIVATE, Modifier.FINAL, Modifier.STATIC)
                .initializer("new $T()", ParameterizedTypeName.get(ClassName.get(HashMap.class), ClassName.get(String.class), className))
                        .build());

        enumBuilder.addMethod(MethodSpec.methodBuilder("get")
                .addParameter(ParameterSpec.builder(ClassName.get(String.class), "value").build())
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .addStatement("return lookup.get($L)", "value")
                .returns(className)
                .build());

        enumBuilder.addStaticBlock(CodeBlock.builder()
                .beginControlFlow("for ($T value:$T.values())", className, className)
                    .addStatement("$L.put(value.getValue(), value)", "lookup")
                .endControlFlow()
                .build());
